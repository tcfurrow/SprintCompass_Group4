...
const ProductBacklogComponent = (props) => {
  ...
  const onAddTaskToDatabaseButtonClicked = async () => {
    try {
      ...
      if (addBacklogTaskResponse !== null && !addBacklogTaskResponse.error) {
        props.showSnackbarMessage(
          `Backlog task added successfully (id: ${addBacklogTaskResponse.backlogTask.id})!`
        );

        const updatedBacklogList = [
          ...state.backlogList,
          addBacklogTaskResponse.backlogTask,
        ];
  ...
  const updateProductBacklogTask = async (
    productBacklogTaskId,
    priority,
    title,
    description,
    relativeEstimate,
    ...
    try {
      props.showSnackbarMessage(
        `Updating backlog task with the id of ${productBacklogTaskId}...`
      );
      ...
      const updateBacklogTaskResponse = await httpUpdate(
        `api/backlog/${productBacklogTaskId}`,
        updatedProductBacklogTask
      );
      ...
      if (updateBacklogTaskResponse?.success) {
        ...
        const backlogIndexToUpdate = updatedBacklogList.findIndex(
          (backlogTask) => backlogTask.id === productBacklogTaskId
        );

        if (backlogIndexToUpdate !== -1) {
          updatedBacklogList[backlogIndexToUpdate] =
            updateBacklogTaskResponse.updatedBacklogTask;
          setState({ backlogList: updatedBacklogList });
        }
      } else {
        props.showSnackbarMessage(
          "Failed to update backlog task due to a server-side issue."
        );
      }
    } catch (error) {
      props.showSnackbarMessage(
        "An error occurred while attempting to update the backlog task."
      );
    }
  };

  const onDeleteProductBacklogTaskButtonClicked = (event, productBacklogId) => {
    const productBacklogTaskToDelete = state.backlogList.find(
      (backlogTask) => backlogTask.id === productBacklogId
    );
  ...
  const deleteProductBacklogTask = async () => {
    ...
    try {
      props.showSnackbarMessage(
        `Deleting product backlog task with the id of ${state.productBacklogTaskToDelete.id})...`
      );

      const deleteBacklogTaskResponse = await httpDelete(
        `api/backlog/${state.productBacklogTaskToDelete.id}`
      );

      if (deleteBacklogTaskResponse?.errorMessage.length > 0) {
        props.showSnackbarMessage(
          `Failed to delete product backlog task. Reason: ${deleteBacklogTaskResponse?.errorMessage}`
        );
      } else {
        if (deleteBacklogTaskResponse.productBacklogTaskDeleted) {
          props.showSnackbarMessage(
            `The product backlog task was deleted successfully!`
          );

          const indexToRemove = state.backlogList.findIndex(
            (backlogTask) =>
              backlogTask.id === state.productBacklogTaskToDelete.id
          );
          ...
          props.showSnackbarMessage(
            `Failed to delete product backlog task due to server-side issue.`
          );
        }
      }
    } catch (error) {
      props.showSnackbarMessage(
        "An error occurred while attempting to delete the product backlog task."
      );
    } finally {
      ...
      <div>
            ...
            <TableBody>
              {state.backlogList.map((productBacklog, index) => (
                ...
                >
                  ...
                  <TableCell component="th" scope="row">
                    <Typography>
                      {currencyFormatter.format(productBacklog.cost)}
                    </Typography>
                  </TableCell>
                  <TableCell component="th" scope="row">
                    <div className="flex-gap">
                      <Button
                        ...
                        onClick={() =>
                          setState({
                            productBacklogTaskToEdit: state.backlogList.find(
                              (backlogTask) =>
                                backlogTask.id === productBacklog.id
                            ),
                          })
                        }
                        variant="outlined"
                      ...
                      <Button
                        ...
                        onClick={(e) =>
                          onDeleteProductBacklogTaskButtonClicked(
                            e,
                            productBacklog.id
                          )
                        }
        ...
        <div className="align-text__left margin-bottom__small">
          <Typography>
            ...
            {currencyFormatter.format(getTotalCost())}
          </Typography>
          <Typography>
            <strong>Total Relative Estimate:</strong>{" "}
